Feature: Equipment Console Tractor Adjustments Process Feature
    ...
    # 37a    #SQL#1 #
  @Regression  @IdentifyINVOICEforTractorReimbursement
  Scenario Outline: Identify INVOICE, for Tractor Reimbursement Process on EQUIPMENT CONSOLE
    Given Locate a Record from Database for Tractor Reimbursement, Agent Status must have a Status of Agent Review or Corp Review <Environment> <TableName1> <CreatedDate>
    Examples:
      | Environment  | TableName1                              | CreatedDate  |
    #  | "ebhlaunch"  | "[Evans].[dbo].[InvoiceRegisterRecord]" | "01/01/2022" |
      | "ebhstaging" | "[Evans].[dbo].[InvoiceRegisterRecord]" | "01/01/2022" |


    # 37b    #SQL#3 #
  @EquipmentConsole  @AgentReimbursement
